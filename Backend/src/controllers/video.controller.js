    ...
    const response = await Video.aggregate([
        ...
        },
        {
            $lookup:{
                ...
                as : "ownerDetails",
                pipeline : [
                    // Look up subscriptions for this specific user/channel
                    {
                        $lookup: {
                            from: "subscriptions",
                            localField: "_id",
                    ...
                    // Project the new fields along with the old ones
                    {
                        $project:{
                            username: 1,
                            fullName: 1,
        ...
        },
        {
            $addFields : {
                ownerDetails : { $first: "$ownerDetails" },
                //Counting the number of elements in the likes array
                likesCount : {$size : "$likes"},
                //Checks if the logged in user has liked the video or not
                isLiked : {
                    $cond:{
        ...
        {
            //Removing the big Likes array from the final response
            $project : {
                likes : 0
            }
        }
